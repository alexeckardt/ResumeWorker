...
def create_prompt(job_description):
    ...
    with open('./prompt.txt', 'r') as f:
        text = f.read().replace('\n', '').strip()
        return text.replace('%JOBDESC', job_description)


def handle_prompt(promptString):
    ...
    x = 'Information retrieval, Distributed computing, Large-scale system design, Networking, Data storage, Security, Artificial intelligence, Natural language processing, UI design, Mobile development, Full-stack development, Leadership, Versatile, Technical infrastructure, Project management'
    splits = x.split(',')
...
def getKeywords():
    ...
    return {
        ...
        'bestkeywords': handle_prompt(prompt)
    }